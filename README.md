...
Streamlit multipage app for evaluating AI response readability and running structured multi-model ratings on CSV datasets with OpenRouter-backed models.

## Run

```powershell
python -m streamlit run readability_evaluator.py
```

## Pages

- `Readability Evaluator`: Upload response JSON, validate structure, compute readability metrics, and optionally run OpenRouter-based extraction.
- `Customer Ratings Studio`: Configure a rating prompt, score CSV rows with two models in parallel, compare agreement, and run a judge model on disagreement rows.
- `Rater Agreement Evaluator`: Upload ratings generated by the Customer Ratings Studio and calculate agreement between two raters.
...
`Customer Ratings Studio` is now a structured rating workflow rather than a simple side-by-side generation demo.
...
## Customer Ratings Studio CSV Requirements
...
## Customer Ratings Studio Output Shape
...
- `Readability Evaluator` can still run validation and readability analysis without an API key.
- `Customer Ratings Studio` Step 3 and Step 4 require an API key.
